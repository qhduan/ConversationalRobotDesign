...
- 任务需要多项要素，例如机票需要时间、起始地、到达地；电影需要名称、电影院、场次时间；咖啡需要时间、大小、口味；
- 任务需要通过多轮对话、多轮反复确认达成
...
学术上基本不区分基于知识的，还是基于知识图谱的（knowledge graph），因为知识图谱是谷歌提出的一个较商业概念。知识在传统上就是三元组，或多元组组成的图结构，而知识图谱也是如此。
...
基本的一问一答模式，即用户提问，没有任何其他信息的，结论为一句话、一个知识点、一个词汇，一个关系的问答。
也可以说是 text-based 的
...
- ！ 易用、易学的QA Pairs管理与查询引擎
    - 机器人管理者简单定义类似FAQ的问答，例如“怎么退货”这种固定回答问题
